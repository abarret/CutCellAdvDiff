...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<CartGridFunction> F_fcn = nullptr;
        Pointer<CellVariable<NDIM, double>> F_var = nullptr;
        bool use_forcing = input_db->getBool("USE_FORCING");
        if (use_forcing)
        {
            F_fcn = new muParserCartGridFunction("F", app_initializer->getComponentDatabase("FFcn"), grid_geometry);
            F_var = new CellVariable<NDIM, double>("F_var");
